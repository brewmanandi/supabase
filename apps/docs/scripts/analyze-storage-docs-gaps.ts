...
interface Report {
  ...
  inCommonSectionsWithoutRef: Array<{ id: string; title: string; slug?: string }>
}
...
async function analyzeStorageDocsGaps(): Promise<Report> {
  ...
  // 4a. Build a set of ALL YAML function IDs (including non-$ref entries like overview sections)
  const allYamlIds = new Set(yamlSpec.functions.map((fn) => fn.id))
  ...
  for (const section of storageSections) {
    if (!yamlById.has(section.id)) {
      // Check if it exists in YAML but without a $ref (e.g., overview sections)
      if (allYamlIds.has(section.id)) {
        report.inCommonSectionsWithoutRef.push(section)
      } else {
...
function printReport(report: Report) {
  ...
  // In common-sections but not in YAML at all
  if (report.inCommonSectionsButNotYaml.length > 0) {
    console.log("❌ IN COMMON-SECTIONS BUT NOT IN YAML (won't display - filtered out by build):")
    ...
    console.log('✅ ALL common-sections entries have corresponding YAML entries\n')
  }

  // In common-sections, exists in YAML but without $ref (overview sections)
  if (report.inCommonSectionsWithoutRef.length > 0) {
    console.log('ℹ️  IN COMMON-SECTIONS WITH YAML ENTRY BUT NO $ref (overview/category sections):')
    console.log(`   ${report.inCommonSectionsWithoutRef.length} sections exist in YAML without $ref\n`)
    report.inCommonSectionsWithoutRef.forEach((section) => {
      console.log(`   - id: ${section.id}`)
      console.log(`     title: ${section.title}`)
      console.log(`     slug: ${section.slug}\n`)
    })
  ...
  console.log(`   In common-sections with YAML but no $ref: ${report.inCommonSectionsWithoutRef.length}`)
  console.log('='.repeat(80) + '\n')
}
